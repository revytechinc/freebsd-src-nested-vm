    ...
    uint64_t gpa)
{
	...
	/*
	 * Information for a VM exit taken during event delivery. If the CPU
	 * took this exit while it was delivering an event into L2, hardware
	 * parked that event here and cleared the entry-interruption valid bit.
	 * That parked copy is the ONLY record the event never arrived: L1
	 * injected it, ENTRY_INTR_INFO reads back as consumed, and L1's own
	 * bookkeeping already treats it as delivered. Hand it up so L1
	 * re-injects it on the next entry.
	 *
	 * An exception would be regenerated by re-executing the instruction,
	 * but an interrupt has already been taken out of L1's vlapic; dropped
	 * here it is gone for good, and an edge-triggered device completion
	 * lost this way hangs L2 with no error recorded anywhere.
	 */
	idtv = vmcs_read(VMCS_IDT_VECTORING_INFO);
	vmcs12_write_field(v12, VMCS_IDT_VECTORING_INFO, idtv);
	vmcs12_write_field(v12, VMCS_IDT_VECTORING_ERROR,
	    (idtv & VMCS_IDT_VEC_VALID) != 0 ?
	    vmcs_read(VMCS_IDT_VECTORING_ERROR) : 0);
}

#define	IDTV_NMI_UNBLOCK_IRET	(1UL << 12)

/*
 * L0 is about to resume L2 from an exit it handled itself. If that exit
 * interrupted the delivery of an event into L2, hardware parked the event in
 * vmcs02's IDT-vectoring field and cleared the entry-interruption valid bit.
 * Nothing else records it -- L1 already considers it delivered -- so move it
 * back into the entry-interruption field and let the next VM entry deliver it.
 * This is what vmx_exit_process() does for a guest L0 runs directly.
 */
static void
vmx_nested_carry_idtv(struct vmx_vcpu *vcpu)
{
	...
	if ((idtv & VMCS_IDT_VEC_VALID) == 0)
		return;
	/*
	 * Refuse to overwrite an event already staged for the next entry.
	 * Nothing stages one on a path that resumes L2 today -- the entry
	 * field was consumed by the entry this exit came from -- but silently
	 * dropping someone else's event while restoring ours would be the same
	 * class of bug this function exists to fix.
	 */
	if ((vmcs_read(VMCS_ENTRY_INTR_INFO) & VMCS_INTR_VALID) != 0)
		return;
	/*
	 * Bit 12 reports NMI-unblocking-due-to-IRET in the IDT-vectoring
	 * field. It is not part of the entry-interruption format and must be
	 * cleared before the value is reused as one.
	 */
	idtv &= ~IDTV_NMI_UNBLOCK_IRET;
	if ((idtv & VMCS_IDT_VEC_ERRCODE_VALID) != 0)
		vmcs_write(VMCS_ENTRY_EXCEPTION_ERROR,
		    vmcs_read(VMCS_IDT_VECTORING_ERROR));
	/*
	 * A software interrupt or software exception is restarted by the CPU,
	 * which needs the length of the instruction that raised it. For a
	 * hardware event the field is ignored, so only set it where it means
	 * something.
	 */
	switch (idtv & VMCS_INTR_T_MASK) {
	case VMCS_INTR_T_SWINTR:
	case VMCS_INTR_T_PRIV_SWEXCEPTION:
	case VMCS_INTR_T_SWEXCEPTION:
		vmcs_write(VMCS_ENTRY_INST_LENGTH,
		    vmcs_read(VMCS_EXIT_INSTRUCTION_LENGTH));
		break;
	}
	vmcs_write(VMCS_ENTRY_INTR_INFO, idtv);
}
...
resume_l2:
	/*
	 * Single exit for every disposition that resumes L2 from an exit L0
	 * handled itself. Any of them can have interrupted event delivery --
	 * an EPT violation or an APIC-access exit parks an event exactly as an
	 * external interrupt does -- so the restore belongs here rather than in
	 * one case, where the next locally-handled exit reason would silently
	 * miss it.
	 */
